...
The search script lives inside this skill's own directory, not the project directory. Codex exposes the absolute path to this `SKILL.md` in the active skill catalog. Resolve `<skill-directory>` as the parent directory of that `SKILL.md`, then invoke the script with its absolute path. Do not rely on agent-specific plugin variables or assume a particular working directory:

```bash
python "<skill-directory>/scripts/search.py" "<query>" --domain <domain>
```

On Windows, prefer `python`, then `py -3`; on macOS/Linux, prefer `python3`, then `python`. Requires Python 3.x and no external dependencies. If the skill path contains spaces, keep it quoted.
