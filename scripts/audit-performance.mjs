...
try {
  ...
  for (const [name, path] of [['accueil', '/'], ['mini-pelle', '/location/mini-pelle']]) {
    const result = await lighthouse(`http://127.0.0.1:4324${path}`, { port: 9222, output: ['json', 'html'], logLevel: 'error', onlyCategories: ['performance', 'accessibility', 'best-practices', 'seo'] });
    ...
    const summary = { page: path, performance: Math.round(result.lhr.categories.performance.score * 100), accessibility: Math.round(result.lhr.categories.accessibility.score * 100), bestPractices: Math.round(result.lhr.categories['best-practices'].score * 100), seo: Math.round(result.lhr.categories.seo.score * 100), lcpMs: Math.round(result.lhr.audits['largest-contentful-paint'].numericValue), cls: result.lhr.audits['cumulative-layout-shift'].numericValue, date: new Date().toISOString() };
